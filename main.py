...
#     sio.emit('message', "WATSON: "+msg+msg)
...
#         print(json.dumps(response, indent=2))
        try:
            ...
            context = response["context"]
            try:
                processed_response, action_flag = watson.processResponse(response)
                ...
                if action_flag == 'pswdreset0':
                    try:
                        ...
                        context = response["context"]
                        try:
                            processed_response, temp = watson.processResponse(response)
                        ...
                        bot_response.append("We're having trouble talking to Watson at the moment. Please try again later")
                        print("Exception Error: ", ex)
            except Exception as ex:
        ...
        if action_flag == 'pswdreset':
            q_count += 1
            next_ques, valid = watson.validateQnA(from_human_message)
    ...
    print(bot_response)
    bot_response[0] = "WATSON: " + bot_response[0]
    cnt = 0
...
#         sio.emit('message', "WATSON: " + bot_response)
        print(bot_response)
        bot_response[0] = "WATSON: " + bot_response[0]
        cnt = 0
        for text in bot_response:
